...
Narrow <-TotalDrillResults %>% gather(PropertyType, NumberOfProjects, TotalProperties:UraniumPercentTotal) %>% #narrow data frame
  filter(!grepl('PercentTotal',PropertyType)) %>% #remove percent total observations. Just want project totals. 
  filter(PropertyType!="TotalProperties") #remove total properties observations. Just want individual commodity totals. 



JustTotal <-TotalDrillResults %>% gather(PropertyType, NumberOfProjects, TotalProperties:UraniumPercentTotal) %>% 
  filter(PropertyType=="TotalProperties") # Filter for TotalProperties to set grey total background

JustTotal2 <-JustTotal[,-2] #create dataframe for totals that exclude property type
